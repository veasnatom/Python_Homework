...
def create_submit_case(request:SubmitCaseDto,get_current_user:UserDto=Depends(get_current_user)):
    try:
...
def update_submit_case(id:int,request: SubmitCaseDto,get_current_user:UserDto=Depends(get_current_user)):
    try:
        validate = ValidateUtils.validateInput(request);
        if validate[0] == True:
            with db_session:
...
def get_all_submit_case(get_current_user:UserDto=Depends(get_current_user)):
    try:
        with db_session:
...
def delete_submit_case(id:int,get_current_user:UserDto=Depends(get_current_user)):
    try:
        with db_session:
